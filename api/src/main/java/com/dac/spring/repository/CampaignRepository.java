...
public interface CampaignRepository extends JpaRepository<CampaignEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "select * from campaign where start_date <= :startDate and end_date > :endDate and statusid= 1 and budget >= bid order by bid desc limit 1,3")
